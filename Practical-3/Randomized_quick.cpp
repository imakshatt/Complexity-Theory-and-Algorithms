    ...
    int partition_low(vector<int> &array, int low, int high)
    {
        ...
        while (true)
        {
            while (start <= end && array[end] >= pivot) // Here i take >= because when i get lower value then pivot so i should stop
            {
                end = end - 1;
            }
            while (start <= end && array[start] <= pivot)//Here i take <= because when i get higher value then pivot so i should stop
            {
                start = start + 1;
            }
            if (start <= end)
