...
void knapsackProblem(map<float, pair<int, int>, greater<float>> &final, int object) {
    ...
    for (auto it : final) {
        if (M > 0 && (it.second.second <= M)) {
            ...
            cout << itemNumber << ") pw_ratio = " << it.first << " is selected with profit = " << selectedProfit << " and weight = " << selectedWeight << " so remaining weight is M = " << M << endl;
            itemNumber++;
        } else {
            if (M > 0) {
                ...
                cout << itemNumber << ") pw_ratio = " << it.first << " is selected with profit = " << selectedProfit << " and weight = " << selectedWeight << " so remaining M = 0" << endl;
                break;
            }
        }
    }
